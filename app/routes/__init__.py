...
from app.models import User, Patient, Doctor, Staff, Appointment, MedicalRecord, Bill
from app.forms import (LoginForm, RegistrationForm, PatientForm, DoctorForm, StaffForm,
                   AppointmentForm, MedicalRecordForm, BillForm, PatientRegistrationForm)
from app.utils import admin_required, doctor_required, get_dashboard_stats, generate_appointment_slots
from app.notifications import Notification
from app.chatbot import Chatbot, ChatMessage
...
def edit_doctor(id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            return redirect(url_for('main.view_doctor', id=doctor.id))
        except Exception as e:
            db.session.rollback()
...
def edit_staff(id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash('An error occurred while updating the staff profile.', 'danger')

    return render_template('staff/edit.html', form=form, staff=staff_member)
...
def edit_appointment(id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            return redirect(url_for('main.view_appointment', id=appointment.id))
        except Exception as e:
            db.session.rollback()
...
def edit_medical_record(id):
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            return redirect(url_for('main.view_medical_record', id=record.id))
        except Exception as e:
            db.session.rollback()
